...
стратегии, который сейчас у `Mob`. Когда стратегию нужно изменить поменять, ему задают новый объект-стратегию.   
...
Наши "продукты" -- различные типы мобов (и стратегий) -- являются объектами одного класса, но отличаются характеристиками объекта (в случае стратегии - реализацией алгоритма).

Поэтому и фабрики производят объекты одного типа, но с существенными различиями в характеристиках. При этом мобы консистентны в рамках одной фабрики (все объекты, производимые фабрикой "сложны одинаково" и отображаются в едином "стиле").
