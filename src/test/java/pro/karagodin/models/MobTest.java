package pro.karagodin.models;

import static org.junit.jupiter.api.Assertions.*;

import org.junit.jupiter.api.Test;
import pro.karagodin.ai_system.PlayerStrategy;
import pro.karagodin.output.DefaultMapStyler;
import pro.karagodin.output.Printer;
import pro.karagodin.time.TimeInterval;

class MobTest {
    @Test
    public void testConstructor() {

        Mob mob1 = new Mob(1, 10, 10, new TimeInterval(100), new PlayerStrategy(new Printer(new DefaultMapStyler())));
        assertEquals(10, mob1.getHp(), 0.001);
        assertEquals(10, mob1.getMaxHp());

        Mob mob2 = new Mob(2, 5, 10, new TimeInterval(10), new PlayerStrategy(new Printer(new DefaultMapStyler())));
        assertEquals(5, mob2.getHp(), 0.001);
        assertEquals(10, mob2.getMaxHp());
    }
}