...
5. **Скорость** (`Delay`) - Время задержки перед следующим действием. Чем меньше тем лучше
* Текущая скорость
...
1. **Опыт** (`XP`)
* Текущий опыт
С достижением определенного значения опыта, увеличивается уровень игрока
...
Эта характеристика для предмета является основной.

Каждый предмет имеет уровень. В зависимости от уровня зависит значение остальных характеристик, 
а также количество параметров, на которые предмет влияет
...
Предметы бывают двух видов - Loot и Consumable. Первое это обычные носимые предметы. 
Второе - это используемые предметы, которые применяют свой эффект и сразу исчезают
...
# 7. Логическая структура <a name="log"></a>
## Диаграмма классов
![](img/classes.png)
